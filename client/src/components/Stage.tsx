...
import {
  ...
  IconFullscreen, IconFullscreenExit, IconClose, IconEye,
} from './Icons';
...
/**
 * O palco: as telas que VOCÊ escolheu assistir, acima da conversa.
 *
 * Voltou na 0.40, depois de duas versões flutuando num canto. A janelinha
 * resolvia o incômodo certo — quem só queria ler o chat não perdia mais
 * metade da janela — mas com ela quem estava assistindo não tinha para onde
 * crescer: o teto do tamanho é a largura do app, e a tela do outro chegava
 * no máximo ainda pequena demais pra acompanhar um jogo.
 *
 * Então a divisão passou a ser por DONO da tela, não por tamanho: a sua vai
 * pra janelinha flutuante (ver Preview.tsx), que é conferência e cabe num
 * canto; a dos outros volta pro palco, que é conteúdo e precisa de área.
 *
 * Quem não quer ver nada não paga nada por isto: o palco só existe pra quem
 * abriu alguma live nos botões da barra de cima, e sem nenhuma aberta ele
 * não renderiza — o chat fica com a janela inteira, como antes.
 */
export function Stage({
  screens, audios, screenVolumes, onScreenVolume, onParar,
}: Props) {
  ...
  return (
    <div
      className={
        'stage' +
        (grande ? ' stage--ampliado' : '') +
        (colunas === 2 ? ' stage--duas' : '')
      }
    >
 ...
 *
 * Três tamanhos, e o clique leva de um pro outro: miniatura (dividindo o
 * palco com a outra) → ampliada (o palco todo, com o chat ainda em algumas
 * linhas embaixo) → tela cheia de verdade, pelo fullscreen do sistema.
 *
 * A escada existe porque cada degrau custa alguma coisa: a ampliada come o
 * chat, e a tela cheia come o app inteiro. Quem quer só acompanhar de canto
 * de olho fica no primeiro; quem foi assistir de verdade desce até o último.
 *
 * Os botões moram no mesmo canto e aparecem juntos no passar do mouse:
 * fechar a live, voltar pra miniatura e tela cheia são a mesma família de
 * decisão, e espalhá-los pela tela era o que deixava o clique longe do que
 * ele afeta.
 */
function Quadro({
  ...
  return (
    <div
      ...
      title={ampliado ? undefined : 'Clique para aumentar; botão direito para o volume'}
    >
      ...
      {/* O clique nos botões não pode subir pro quadro: ele aumentaria a
